...
type TGBGpu = class
  private
    ...
    LcdStatus_isAnyStat,LcdStatus_lylycEnable,LcdStatus_oamAccessEnable,
    LcdStatus_vblankEnable,LcdStatus_hblankEnable,LcdStatus_isLyLyc: Boolean;

    LcdControl_lcdEnable,LcdControl_wndTileMapDisplaySelect,LcdControl_wndDisplayEnable,
    LcdControl_bgAndWndTileDataSelect,LcdControl_bgTileMapDisplaySelect,LcdControl_tallSpriteMode,
    LcdControl_spriteDisplayEnable,LcdControl_bgWndDisplayPriority: Boolean;
...
var
  newLcdEnable,newWndTileMapDisplaySelect,newWndDisplayEnable,
  newBgAndWndTileDataSelect,newBgTileMapDisplaySelect,newTallSpriteMode,
  newSpriteDisplayEnable,newBgWndDisplayPriority: Boolean;
begin
  if (value and $80 <>0) then
    newLcdEnable := True
  else
...
begin
  ...
  case currentMode of
    ...
    VBLANK:begin
      if (modeClock >= 456) then begin
        modeClock := 0;
        line := line + 1;
      end;
      if (line > 153) then begin
        currentMode := Mode.OAM_ACCESS;
        line := 0;
      end;
    end;
