...
interface
uses SysUtils, GBInterruptManager;
...
begin
  if FInstance = nil then FInstance := TGBTimer.Create;
  Result := FInstance;
end;
...
begin
  ...
  begin
    ...
    begin
      // 还是要实现中断
      GBInterrupt.Instance.raiseInterruptByIdx(2);// 'TIMER_OVERFLOW';
    end;
    if (_ticksSinceOverflow = 5) then
      _tima := _tma;
